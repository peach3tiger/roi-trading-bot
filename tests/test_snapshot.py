"""Smoke test — backtest 7 ngày, config MẶC ĐỊNH, so với snapshot đã commit.
...
| `test_snapshot.py` (file này) | backtest 7 ngày, dữ liệu THẬT, config THẬT | ~10s | mỗi commit |
...
gì đó vỡ" trong 10 giây thay vì 137 — feature tính sai, HMM train khác,
...
Giá của điều đó là 10 giây. Chấp nhận — nó vẫn nhanh hơn harness 13 lần.
...
## Giới hạn đã biết của cửa sổ này

Chuỗi regime trong 7 bar được chọn là HẰNG SỐ (`STRONG_BEAR` × 8), nên
phép so chuỗi regime ở đây là tín hiệu YẾU — nó bắt được việc nhãn đổi
hẳn, không bắt được lỗi ở logic CHUYỂN regime. Phần đó do
`regression_harness.py` gánh (`regime_transitions`, ngưỡng 2%). Ghi ra để
không ai tưởng file này phủ được chuyển regime.
"""
...
def smoke_result() -> Any:
    """Chạy backtest MỘT LẦN cho cả file — ~10s là chi phí một lần, không
    phải mỗi assert."""
...
def test_chuoi_regime_khop(smoke_result: Any, snapshot: dict[str, Any]) -> None:
    """Xem "Giới hạn đã biết" ở docstring module: chuỗi này là hằng số
    trong cửa sổ đã chọn, nên đây là tín hiệu YẾU cho logic chuyển regime."""
